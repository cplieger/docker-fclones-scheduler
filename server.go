...
// run request (see protocol.go). The socket replaces the exec-child
// execution path of the previous design: mutual exclusion is the executor's
// single goroutine, shutdown reaches waiting clients as explicit
// cancellation results, and every run's log lines land on the container's
// own log stream in every mode because the run executes in the daemon. The
// /cache flock survives inside runFclonesJob only as the cross-container
// guard.
...
// fails on an existing path otherwise).
func listenTrigger(path string) (net.Listener, error) {
	...
	// at connect for a mismatched exec user (the old shape lost only the
	// health heartbeat, silently).
