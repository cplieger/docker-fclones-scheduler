...
// run request and blocks until the daemon reports the run result, exiting
// 0/1 with that result. The external trigger contract is unchanged from the
// previous design (`docker exec fclones /app/wrapper scan`, exit code = run
// outcome, marker updated), but the run itself now executes inside the
// daemon: its logs land on the container's log stream in every mode, and
// this process's output is only its own lifecycle lines. The client never
// touches the health marker — the daemon is its single writer — so the old
// silent-heartbeat-loss footgun (a mismatched exec user failing the marker
// write) is structurally gone; a mismatched user now fails loudly at
// connect (the socket is owner-only).
...
// Ofelia job log) — while the run's full output lands in the container log
// stream.
